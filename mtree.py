...
class MTree(object):
    ...
    def search_in_range(self, query_obj, distance):
        ...
        #at the end will contain the results 
        nn = NN(k)
        #fix the search radius so that it will only contain results within this search space
        nn.fix_search_radius(distance)
...
class NN(object):
    ...
    def fix_search_radius(self, dmax):
    ...
    def result_list(self):
        result = [entry.obj for entry in self.elems if entry.obj]
        return result
    ...
    def search(self, query_obj, pr, nn, d_parent_query):
        for entry in self.entries:
            if self.could_contain_results(query_obj,
                ...
                if distance_entry_to_q == 0:
                    continue
                if distance_entry_to_q <= nn.search_radius():
                    nn.update(entry.obj, distance_entry_to_q)

    def draw(self):
        for rout_obj in self.entries:
            obj = rout_obj.obj
            # plt.text(obj.x - len(obj.name) / 3.5, obj.y + 1, obj.name, fontsize = 10)
...
class InternalNode(AbstractNode):
    ...
    def draw(self):
        for rout_obj in self.entries:
            ...
            if r > 0:
                obj = rout_obj.obj
                x = np.linspace(-obj.x - r, obj.x + r, 10000)
                y = np.linspace(-obj.y - r, obj.y + r, 10000)
                X, Y = np.meshgrid(x, y)
                F = (X - obj.x) ** 2 + (Y - obj.y) ** 2 - r ** 2
                plt.contour(X, Y, F, [0])
            rout_obj.subtree.draw()
